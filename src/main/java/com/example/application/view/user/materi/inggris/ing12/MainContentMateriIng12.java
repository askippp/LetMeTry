...
@Route("materi-ing-12")
@PageTitle("Materi - Inggris - 12")
public class MainContentMateriIng12 extends VerticalLayout implements BeforeEnterObserver, MateriClickListener {

    private VerticalLayout mainContent;

    public MainContentMateriIng12() {}
    ...
    private void buildLayout() {
        ...
        HorizontalLayout header = Header.headerWithBackButton("Inggris kelas 12");
        ...
        onMateriClick("Active and Passive Voice");
    }
    ...
    private VerticalLayout createMainContent() {
        ...
        List<ModelMateriIng> daftarMateri = DaftarMateriIng12.getDaftarMateriIng12();

        for (ModelMateriIng materiIng : daftarMateri) {
            mainContent.add(createMateriCard(materiIng));
        }

        return mainContent;
    }

    private VerticalLayout createMateriCard(ModelMateriIng materiIng) {
        ...
        H2 judul = new H2(materiIng.getJudul());
        ...
        btnLatsol.addClickListener(buttonClickEvent -> {
            try {
                if (materiIng.getIdMateri() > 0) {
                    UI.getCurrent().navigate("latsol-ing-12/" + materiIng.getIdMateri());
                } else {
        ...
        Paragraph pengertian = new Paragraph(materiIng.getPengertian());
        ...
        for (String contoh : materiIng.getContoh()) {
        ...
        for (String p : materiIng.getPenjelasan()) {
    ...
    public void onMateriClick(String judul) {
        ...
        ModelMateriIng materiTerpilih = DaftarMateriIng12.getDaftarMateriIng12().stream()
