package com.example.application.view.user.materi.indonesia.ind10;
...
import com.example.application.view.user.materi.indonesia.ModelMateriInd;
...
@Route("materi-ind-10")
@PageTitle("Materi - Indonesia - 10")
public class MainContentMateriInd10 extends VerticalLayout implements BeforeEnterObserver, MateriClickListener {

    private VerticalLayout mainContent;

    public MainContentMateriInd10() {}
    ...
    private void buildLayout() {
        ...
        HorizontalLayout header = Header.headerWithBackButton("Indonesia kelas 10");
        ...
        onMateriClick("Teks Laporan Hasil Observasi");
    }
    ...
    private VerticalLayout createMainContent() {
        ...
        List<ModelMateriInd> daftarMateri = DaftarMateriInd10.getDaftarMateriInd10();

        for (ModelMateriInd materiInd : daftarMateri) {
            mainContent.add(createMateriCard(materiInd));
        }

        return mainContent;
    }

    private VerticalLayout createMateriCard(ModelMateriInd materiInd) {
        ...
        H2 judul = new H2(materiInd.getJudul());
        ...
        btnLatsol.addClickListener(buttonClickEvent -> {
            try {
                if (materiInd.getIdMateri() > 0) {
                    UI.getCurrent().navigate("latsol-ind-10/" + materiInd.getIdMateri());
                } else {
        ...
        Paragraph pengertian = new Paragraph(materiInd.getPengertian());
        ...
        for (String contoh : materiInd.getContoh()) {
        ...
        for (String p : materiInd.getPenjelasan()) {
            penjelasan.add(new Paragraph(p));
        }

        card.add(topLayout, pengertianTitle, pengertian, contohTitle, listContoh, penjelasanTitle, penjelasan);
        return card;
    }
    ...
    public void onMateriClick(String judul) {
        ...
        ModelMateriInd materiTerpilih = DaftarMateriInd10.getDaftarMateriInd10().stream()
