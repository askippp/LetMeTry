...
@Route("tryout-level2/indonesia")
@PageTitle("TryOut - Indonesia")
public class IndonesiaMedium extends VerticalLayout implements BeforeEnterObserver {
    ...
    public void beforeEnter(BeforeEnterEvent event) {
        ...
        if (user == null) {
            ...
            Notification.show("Point anda kurang! Butuh 200 point untuk ke level ini!", 2000, TOP_CENTER);
            ...
            try {
                ...
                idToMedium = new TryOutMediumDAO().startTryOut(usersModel.getIdUsers(), idMapel);
    ...
    private void displayQuestion(int questionNumber) {
        ...
        sortedAnswers.forEach((option, answer) -> {
            ...
            int idJawabanToMedium = currentQuestion.stream()
                    .filter(q -> q.getOpsi().equalsIgnoreCase(option))
                    .findFirst()
                    .map(TryOutMediumModel::getIdJawabanToMedium)
                    .orElse(-1);

            answerButton.addClickListener(e -> {
                jawabanUser.put(currentQuestion.getFirst().getIdSoalToMedium(), idJawabanToMedium);

                try {
                    new TryOutMediumDAO().saveJawaban(
                            idToMedium, currentQuestion.getFirst().getIdSoalToMedium(), idJawabanToMedium
                    );

                    boolean isCorrect = new TryOutMediumDAO().isJawabanBenar(idJawabanToMedium);
                    if (isCorrect) {
                        Notification
    ...
    private void handleSubmit() {
        try {
            ...
            new TryOutMediumDAO().finishTryOut(idToMedium, waktuPengerjaan);

            Notification.show("TryOut berhasil diselesaikan!", 2000, MIDDLE)
                    .addThemeVariants(NotificationVariant.LUMO_SUCCESS);

            UI.getCurrent().navigate("hasil-tryout/" + idToMedium);
