...
import com.example.application.dao.TryOutHardDAO;
...
@Route("try-out-hard")
@PageTitle("TryOut Hard - Admin")
public class ManageTryOutHard extends VerticalLayout implements BeforeEnterObserver {

    private final TryOutHardDAO tryOutHardDAO = new TryOutHardDAO();
    private final MateriDAO materiDAO = new MateriDAO();

    private Grid<TryOutHardModel> soalGrid;
    private Integer selectedMapel;
    private List<TryOutHardModel> currentSoalList = new ArrayList<>();

    public ManageTryOutHard() {}
    ...
    private VerticalLayout createMainContent() {
        ...
        H2 title = new H2("TryOut Hard Management");
    ...
    private HorizontalLayout createLevelNavigation() {
        ...
        Button levelHard = new Button("Hard");
    ...
    private int getQuestionCountForMapel(int idMapel) {
        try {
            // Use existing method from TryOutHardDAO
            List<TryOutHardModel> soalList = tryOutHardDAO.getTryOutHardByIdMapel(idMapel);
            return (int) soalList.stream()
                    .map(TryOutHardModel::getIdSoalToHard)
                    .distinct()
                    .count();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private VerticalLayout createSoalSection() {
        ...
        H3 soalTitle = new H3("Daftar Soal TryOut Hard");
    ...
    private Grid<TryOutHardModel> createSoalGrid() {
        Grid<TryOutHardModel> grid = new Grid<>(TryOutHardModel.class, false);
        grid.setHeight("400px");

        grid.addColumn(TryOutHardModel::getNoSoal).setHeader("No").setWidth("80px");
        ...
        grid.addColumn(TryOutHardModel::getOpsi).setHeader("Kunci Jawaban").setWidth("80px");
    ...
    private void loadSoalData(int idMapel) {
        try {
            currentSoalList = tryOutHardDAO.getTryOutHardByIdMapel(idMapel);

            // Get unique questions with correct answers
            List<TryOutHardModel> soalYangBenar = currentSoalList.stream()
    ...
    private void openSoalDialog(TryOutHardModel soal) {
        if (selectedMapel == null) {
    ...
    private void populateFieldsFromSoal(
        ...
        List<TryOutHardModel> jawabanList = currentSoalList.stream()
                .filter(s -> s.getNoSoal() == soal.getNoSoal() && s.getIdMapel() == soal.getIdMapel())
                .sorted(Comparator.comparing(TryOutHardModel::getOpsi))
                .collect(Collectors.toList());

        for (int i = 0; i < optionFields.size(); i++) {
            String option = String.valueOf((char) ('A' + i));
            TryOutHardModel jawaban = jawabanList.stream()
    ...
    private void setDefaultValuesForNewSoal(
        ...
        int nextNo = currentSoalList.stream()
                .mapToInt(TryOutHardModel::getNoSoal)
                .max()
                .orElse(0) + 1;
    ...
    private boolean validateSoalForm(
        ...
        if (valid && noSoal.getValue() != null) {
            try {
                boolean isDuplicate = tryOutHardDAO.isQuestionNumberUsed(
                        noSoal.getValue(),
                        selectedMapel,
                        existingSoal != null ? existingSoal.getIdSoalToHard() : null
                );
                if (isDuplicate) {
    ...
    private void saveSoal(
        ...
        try {
            if (existingSoal == null) {
                 tryOutHardDAO.addSoalWithAnswers(noSoal, selectedMapel, idMateri, pertanyaan, optionFields, correctOption);
                 Notification.show("Soal berhasil ditambahkan!", 3000, Notification.Position.MIDDLE);
            } else {
                 tryOutHardDAO.updateSoalWithAnswers(existingSoal.getIdSoalToHard(), noSoal, pertanyaan, optionFields, correctOption);
    ...
    private void confirmDelete(TryOutHardModel soal) {
        ...
        dialog.addConfirmListener(e -> {
            try {
                 tryOutHardDAO.deleteSoalWithAnswers(soal.getIdSoalToHard());
